...
- *Window managers* — Hyprland, Sway, river, qtile, awesome, xmonad, leftwm, and
  the suckless stack (dwm, dwl, dmenu, st, dwmblocks) under =config/suckless/=.
...
The =config/= and =emacs/= trees are plain dotfiles and work on any distro —
symlink (or stow) the ones you want into place. The suckless programs
(under =config/suckless/=) ship with their sources and patches, so build them
with =make && sudo make install= inside each directory.
