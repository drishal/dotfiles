...
Item {
    ...
    // ── Gateway auth key ───────────────────────────────────────
    // The Hermes gateway protects every endpoint except /health with a bearer
    // token (API_SERVER_KEY in ~/.hermes/.env). If the user hasn't set one in
    // Settings, auto-discover it from the env file so the app works out of the
    // box on a local gateway. An explicit apiKey always wins.
    property string _envApiKey: ""
    readonly property string _effectiveKey: apiKey || _envApiKey
    readonly property string _authVal: _effectiveKey ? "Authorization: Bearer " + _effectiveKey : ""

    Process {
        id: envKeyProcess
        running: false
        property string envPath: ""
        // Grab the value of API_SERVER_KEY, tolerating optional quotes/export.
        command: ["sh", "-c",
            "f=\"${ENV_PATH/#\\~/$HOME}\"; [ -f \"$f\" ] && sed -n 's/^[[:space:]]*\\(export[[:space:]]\\+\\)\\?API_SERVER_KEY=//p' \"$f\" | head -1 | sed 's/^[\"'\\'']//;s/[\"'\\'']$//'"
        ]
        environment: ({ "ENV_PATH": envPath })
        stdout: StdioCollector {
            onStreamFinished: root._envApiKey = (text || "").trim()
        }
    }

    function _loadEnvApiKey() {
        if (apiKey) return            // explicit key set — no need to discover
        envKeyProcess.envPath = (hermesHome || "~/.hermes") + "/.env"
        envKeyProcess.running = false
        envKeyProcess.running = true
    }

    onHermesHomeChanged: _loadEnvApiKey()
    onApiKeyChanged: { if (!apiKey) _loadEnvApiKey() }
