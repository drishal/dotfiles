...
| `themes/`                              | Theme JSON; `stylix.json` is generated by Home Manager | Yes (step 4) |
...
Themes ship in this repo at `config/pi/agent/themes/`: `gruvbox-material.json`
(default) and `claude-dark.json`. A third, `stylix.json`, is **not** stored
there — Home Manager generates it into `~/.pi/agent/themes/` from
`NixOS/home/common/core/pi-theme.nix`. Install one by hand with:

```bash
mkdir -p ~/.pi/agent/themes
cp ~/dotfiles/config/pi/agent/themes/gruvbox-material.json ~/.pi/agent/themes/
```
