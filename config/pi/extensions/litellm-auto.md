...
A local extension that discovers chat models from a LiteLLM-compatible gateway
automatically, so you never hand-write the model list.
...
const CREDENTIALS_DIR = join(homedir(), ".pi", "secrets");

async function readCredential(name: string): Promise<string | undefined> {
  try {
    const value = (await readFile(join(CREDENTIALS_DIR, name), "utf8")).trim();
...
export default async function (pi: ExtensionAPI) {
  // resolution order: env var > credential file (see below)
  const baseUrl =
    process.env.LITELLM_BASE_URL ?? (await readCredential("litellm.url")) ?? "";
  const apiKey =
    process.env.LITELLM_API_KEY ?? (await readCredential("litellm.key")) ?? "";
...
| Env var                | Purpose                                                                       |
| ---------------------- | ----------------------------------------------------------------------------- |
| `LITELLM_BASE_URL`     | Gateway root, e.g. `https://gateway.internal/v1` — or the credential file     |
| `LITELLM_API_KEY`      | Bearer token — or the credential file                                         |
...
Credentials: the URL and key can live in plain files such as
`~/.pi/secrets/litellm.url` / `litellm.key` (one value per file, `chmod 600`).
The extension reads them as a fallback after the env vars.
