...
- **Embeddings:** Qwen3-Embedding-8B @ 4096 dims via the litellm gateway (`~/.pi/secrets/litellm.url`) — configured in the MCP server env, already working. Stored as `int8[4096]` (quantized, 4KB/vector).
...
- Good: "User prefers NixOS + Neovim; pi runs on GLM5 via the litellm gateway (URL in ~/.pi/secrets/litellm.url)"
