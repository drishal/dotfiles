...
- **quickshell services (caelestia-derived)** — `Notif.qml` persists unclosed notifications to `~/.local/state/quickshell/notifs.json` (1s debounce; restored entries are inert snapshots — actions are no-ops) and DND survives reloads via `PersistentProperties`; popup + toast stacks hide on fullscreen (`Notif.hasFullscreen()`, criticals break through). `Toaster` is the internal toast stack (`Services/Toaster.qml` + `Modules/Toasts.qml`, bottom-right on the overlay layer) for shell-own events — battery warnings, DND flips, idle locks — never route those through notifd; fire over IPC: `qs ipc call -- toast show "T" "B"`. `Idle.qml` locks after 10 min (Mpris-playing inhibit, logind `PrepareForSleep` → lock); **no DPMS and no automatic suspend anywhere — sleeping is manual only**. `Brightness.qml` is per-device (`backlight` via brightnessctl + `ddc` via ddcutil, detect once at startup; ddc needs `i2c-dev` in boot.kernelModules, already on nixos-work). `Players.qml` owns player selection + `artUrl()` YouTube-thumbnail fallback for mpv-mpris. **Every QML file starts with `pragma ComponentBehavior: Bound`**; delegate properties must be null-guarded (`modelData` goes null transiently during ScriptModel recycle — read nested fields through a guarded `n` property, see NotificationCenter). Shell env pragmas live in `shell.qml` (`QSG_RENDER_LOOP=threaded` etc.); a change to them needs `qs kill; qs`, not just hot reload.
