...
- **`home/common/default.nix` is the orchestrator** — imports individual files from `core/`, `desktop/`, `editors/`, `terminals/`, plus `shells/default.nix`, `browsers/betterfox.nix`, `media/mpv.nix`, `media/yt-dlp.nix`, and both stylix modules. Does NOT import whole `core/`, `browsers/`, or `media/` directories.
...
- **The login shell is fish, so remote tooling that sends POSIX snippets breaks** — `users.users.drishal.shell = pkgs.fish`, and sshd runs every non-interactive command through it. Herdroid (Android client for herdr) verifies its uploaded bridge with a bare `hash=$(sha256sum -- ...) || exit; set -- $hash` — fish answers `Unsupported use of '='` and the app reports `bridge_install_failed` even though the binary landed with the right hash. `hosts/common/base.nix` therefore gives the phone's tailscale address a `ForceCommand` shim (`sshPosixShim`) that execs bash for commands, the sftp-server for the subsystem (herdroid uploads over SFTP, so the shim must handle it) and fish for interactive logins. Match on the address, not `User` alone — a blanket ForceCommand would change every SSH session on both hosts.
- **Screenshots: `capture` on Hyprland, `grimshot` on sway** — grimshot finds the active output with `swaymsg -t get_outputs`, which has no socket under Hyprland, so it silently drops grim's `-o` and composites *every* monitor. `scripts/capture.sh` (wrapped in `home/common/desktop/hyprland.nix`) asks hyprctl instead and also owns the OCR bind. sway keeps grimshot — it works correctly there, so don't unify them.
...
- **quickshell workspaces come from ext-workspace-v1, not `Quickshell.Hyprland`** — Hyprland 0.56 ([#16140](https://github.com/hyprwm/Hyprland/pull/16140)) removed workspace ids from its IPC in favour of `address` + `type`, across `workspaces -j`, `monitors -j`, `clients -j` and the socket2 `*v2` events. quickshell still parses `"id"`, so every workspace reads back as id -1 and collapses onto one phantom object ([quickshell#1119](https://github.com/quickshell-mirror/quickshell/issues/1119)). `Modules/Workspaces.qml` therefore reads `WindowManager.windowsetProjections` (`Quickshell.WindowManager`), which is compositor-agnostic and unaffected; `Windowset.activate()` replaces the click dispatch, `urgent` replaces the hand-rolled tracking, and `windowsetProjections` gives the per-monitor split. It exposes no window count, but Hyprland only keeps a workspace alive while it holds windows or is focused, so every reported workspace earns a slot. The wheel handler still uses `Hyprland.dispatch` — a command, not a query, so the id removal doesn't touch it. `Hyprland.monitors` is still fine (monitors kept their `id`); only anything reached *through* a workspace is not. **The raw `lastIpcObject` is always current** — it is quickshell's own object model that is stale — so `Notif.hasFullscreen()` matches `toplevel.lastIpcObject.workspace.address` against each monitor's `activeWorkspace.address` rather than walking `monitor.activeWorkspace.toplevels`. `Hyprland.toplevels` is created on first use like any singleton list, so Notif holds it live in a binding; reading it only from inside a function leaves it permanently empty.
- **quickshell bar cells are left/right click, never hover** — a `Cell` in `Bar.qml` takes `popout` (left click) and `panel` (right click); opening on hover fired constantly just from crossing the bar. Left click opens one shared popout (`Modules/Popouts/PopoutHost.qml`) that morphs between contents rather than closing and reopening: a `Spring` drives x/width/height, the bodies cross-fade. All bodies stay instantiated and gate their work on `live` — building one on demand put its construction (WifiDetail plus an nmcli spawn) in the middle of the morph. The popout stands down whenever a panel owns the screen, and `ShellSurface`'s focus grab dismisses it on an outside click. Right click opens the cell's panel: network → dashboard, CPU/RAM → `ProcessList`, which grows out of the cluster via `Panel.morphFrom` instead of appearing under the clock.
- **quickshell process list** — `Services/Procs.qml` samples `/proc/<pid>/stat` twice and diffs against the machine total, because `ps` only reports CPU averaged over a process's lifetime. Percentages are per-core, like htop. Sampling is ref-counted (`addRef`/`removeRef`), so it only runs while a panel is open. **Long lists are `FadeListView` + `ScriptModel`, never a `Repeater`** (this applies to the process table, the notification list and the clipboard) — a Repeater rebuilds every delegate whenever the model array changes, so with ~900 processes each keystroke and each 3s poll reconstructed hundreds of rows; the ListView builds only what is visible and `ScriptModel { objectProp: "pid" }` diffs so a refresh updates rows in place. Search is debounced, and `Procs.list` is dropped when the last ref goes.
- **quickshell memory rules** — a `Repeater` over an unbounded list builds every delegate at once and never releases them, which is how the shell used to reach hundreds of MB in a long session: `Notif.list` grew all session with a full-resolution `Image` per entry. So: lists use `FadeListView` + `ScriptModel`, **every `Image` sets `sourceSize`** (a screenshot notification decoded at 1920×1080 for a 34px avatar), `Notif.list` is capped at `maxList`, and services that spawn processes are ref-counted (`Net`, `Radios`, `Procs`) so idle polling backs off when nothing is displaying them. Measure with `Pss_Anon` from `/proc/<pid>/smaps_rollup` — note `qs` forks, so pick the `quickshell` process, not the launcher.
- **quickshell owns the session lock** — `Modules/LockScreen.qml` (`WlSessionLock` + PAM) with visuals split into `LockSurface.qml` (composition) + `Modules/Lock/` (the cards) so they can be instantiated for testing without locking — `qs -p config/quickshell/lock-preview.qml` renders the surface in a plain window. It is a three-column dashboard: weather/host/media left, clock+avatar+password centre, CPU-RAM-swap + notifications right; the side columns show only on the focused monitor and collapse to a status strip below 1280x760. `Lock/NotifDock.qml` deliberately stays PlainText where the notification centre uses Markdown — it renders untrusted text pre-auth. **Requires `security.pam.services.quickshell` (hosts/common/users.nix)**; `Services/LockState.qml` checks for `/etc/pam.d/quickshell` and falls back to swaylock if it is missing, so a not-yet-switched system cannot lock you out. `loginctl lock-session` drives it — LockState listens for the logind D-Bus signal via `dbus-monitor` — so the SUPER+SHIFT+L bind, the power menu and the dashboard all take one path. Also `qs ipc call lock lock`.
...
- **quickshell services (caelestia-derived)** — `Notif.qml` persists unclosed notifications to `~/.local/state/quickshell/notifs.json` (1s debounce; restored entries are inert snapshots — actions are no-ops) and DND survives reloads via `PersistentProperties`; popup + toast stacks hide on fullscreen (`Notif.hasFullscreen()`, criticals break through; `Toaster` calls Notif's copy rather than keeping its own). **Clear all only commits when its stagger timer fires, so closing the panel must finish a pending clear — it used to cancel it and leave every notification in place** (stagger is capped at 12 cards × 50ms for exactly that reason), and entries are pruned from each notification's own `closed` signal connected at insert, not from a delegate: the Instantiator handler read `modelData` at signal time, which is undefined once the delegate is recycled, so it threw and stranded the notification in `list`. **Notification expiry is honoured** — quickshell surfaces the client's raw ms `expireTimeout` and nothing else acts on it (libnotify leaves the timeout to the server), so `expireTimeout > 0` now calls `expire()` while `0`, `-1` and criticals stay put. `Toaster` is the internal toast stack (`Services/Toaster.qml` + `Modules/Toasts.qml`, bottom-right on the overlay layer) for shell-own events — battery warnings, DND flips, idle locks — never route those through notifd; fire over IPC: `qs ipc call -- toast show "T" "B"`. `Idle.qml` locks after 10 min (Mpris-playing inhibit, logind `PrepareForSleep` → lock); **no DPMS and no automatic suspend anywhere — sleeping is manual only**. `Idle` and `BatteryMonitor` have no other consumer and QML creates singletons on first use, so **`shell.qml` must touch them or they never run** — a singleton nothing references is silently dead, and its log stays clean. `Brightness.qml` splits by source, not by monitor (`backlight` via brightnessctl + one `ddc` target via ddcutil, detected once at startup) — the DDC probe only establishes that some display answered and drives ddcutil's default display, so it is **not** per-monitor yet. DDC needs `/dev/i2c-*` access: `hardware.i2c.enable` (hosts/common/gui.nix) loads i2c-dev *and* installs the uaccess udev rule — loading the module alone leaves the nodes root-only and ddcutil fails silently for the user. `Players.qml` owns player selection + `artUrl()` YouTube-thumbnail fallback for mpv-mpris. **Every QML file starts with `pragma ComponentBehavior: Bound`**; delegate properties must be null-guarded (`modelData` goes null transiently during ScriptModel recycle — read nested fields through a guarded `n` property, see NotificationCenter). Shell env pragmas live in `shell.qml` (`QSG_RENDER_LOOP=threaded` etc.); a change to them needs `qs kill; qs`, not just hot reload.
...
- **aria2 connection count is tuned down, not up** — `home/common/core/aria2.nix` uses `split=8` / `max-connection-per-server=8` / `min-split-size=4M`, not the usual 16/16/1M. Benchmarked on this line (Vi India ~300/55 Mbps, 27 ms RTT, 1 GB from sin-speed.hetzner.com): 1 conn 117 Mbps, 4-8 conns 180-224, 16 conns 140-155. Past ~4 connections there is no gain and 16 is consistently slower, because aria2 re-splits the tail of the file into ever smaller ranges — `min-split-size=1M` at 16 conns measured 154 Mbps vs 220 at 8M. Raise per-download with `-x16 -s16` only for mirrors that throttle per-connection. `max-tries=0` is deliberately avoided: with a short `retry-wait` it spins forever on a dead URL and blocks the queue. **`max-file-not-found` must stay unset** — it counts dead HTTP *web seeds* embedded in a .torrent and aborts the whole download group when it trips; the Debian DVD torrent ships a stale cdimage URL and died at `count=3` with 8 peers connected and a healthy swarm. **No `optimize-concurrent-downloads`** — it is clamped *below* `max-concurrent-downloads`, and its `N = 5 + 25*log10(Mbps)` passes 4 at ~40 Mbps, so with a cap of 4 it can only ever reduce the count. **No RPC daemon** — `systemd.enable` is left off because it unconditionally passes `--enable-rpc`.
- **aria2 torrent tuning is separate from the HTTP tuning** — `split`/`max-connection-per-server`/`min-split-size`/`piece-length`/`lowest-speed-limit` are HTTP/FTP-only and documented as ignored or inapplicable for BitTorrent; only `disk-cache`, `file-allocation`, `max-tries`/`timeout`, and the `max-*-limit` pair carry over. Measured: torrents reach 310-360 Mbps (line rate) where single-server HTTP capped at 224, so HTTP is route-limited, not line-limited. `bt-max-peers` stays 100 — real swarms topped out at 58-59 peers, and a 9x gap first attributed to raising it to 200 was a **cold DHT routing table** on the first run; re-tested with warm DHT, 100 and 200 were within noise. **aria2 has no UPnP/NAT-PMP and no uTP** — behind NAT it gets zero inbound peers (verified: 0 conns to the listen port, 50 established outbound, 15 of them IPv6), so being connectable needs a manual router forward of TCP+UDP 51413.
- **yt-dlp's `downloader-args` must live in `extraConfig`** — `home/common/media/yt-dlp.nix`. HM renders `settings` values unquoted, and yt-dlp parses its config with shlex, so a value containing spaces is split into stray arguments. Booleans there also render as `--no-<name>` when false, so a `false` on a `no-`-prefixed option produces `--no-no-mtime`. Fragments override aria2.conf to `--file-allocation=none` — HLS/DASH is many small files, so falloc and the 128M disk cache are wasted.
