...
- **quickshell process list** — `Services/Procs.qml` samples `/proc/<pid>/stat` twice and diffs against the machine total, because `ps` only reports CPU averaged over a process's lifetime. Percentages are per-core, like htop. Sampling is ref-counted (`addRef`/`removeRef`), so it only runs while a panel is open. **The table is a `FadeListView` + `ScriptModel`, never a `Repeater`** — a Repeater rebuilds every delegate whenever the model array changes, so with ~900 processes each keystroke and each 3s poll reconstructed hundreds of rows; the ListView builds only what is visible and `ScriptModel { objectProp: "pid" }` diffs so a refresh updates rows in place. Search is debounced.
